...
(function (global, factory) {
	...
	(global.VueAudioMotionAnalyzer = factory());
...
class AudioMotionAnalyzer {
  ...
  */
	constructor(container, options = {}) {
		...
		this._gradients = {
			classic: {
				bgColor: '#111',
				colorStops: ['hsl( 0, 100%, 50% )', { pos: .6, color: 'hsl( 60, 100%, 50% )' }, 'hsl( 120, 100%, 50% )']
			},
			prism: {
				bgColor: '#111',
				colorStops: ['hsl( 0, 100%, 50% )', 'hsl( 60, 100%, 50% )', 'hsl( 120, 100%, 50% )', 'hsl( 180, 100%, 50% )', 'hsl( 240, 100%, 50% )']
			},
			rainbow: {
				bgColor: '#111',
				dir: 'h',
				colorStops: ['hsl( 0, 100%, 50% )', 'hsl( 60, 100%, 50% )', 'hsl( 120, 100%, 50% )', 'hsl( 180, 100%, 47% )', 'hsl( 240, 100%, 58% )', 'hsl( 300, 100%, 50% )', 'hsl( 360, 100%, 50% )']
			}
		};
		...
		if (options.hasOwnProperty('audioCtx')) {
			if (options.audioCtx instanceof AudioContext) this._audioCtx = options.audioCtx;else throw new AudioMotionError('ERR_INVALID_AUDIO_CONTEXT', 'Provided audio context is not valid');
		} else {
			try {
  ...
  */
	...
	set gradient(value) {
		if (this._gradients.hasOwnProperty(value)) this._gradient = value;else throw new AudioMotionError('ERR_UNKNOWN_GRADIENT', `Unknown gradient: '${value}'`);
	}
	...
	set mode(value) {
		...
		if (mode >= 0 && mode <= 10 && mode != 9) {
			this._mode = mode;
			this._precalculateBarPositions();
			if (this._reflexRatio > 0) this._generateGradients();
		} else throw new AudioMotionError('ERR_INVALID_MODE', `Invalid mode: ${mode}`);
	}
	...
	set reflexRatio(value) {
		value = Number(value);
		if (value < 0 || value >= 1) throw new AudioMotionError('ERR_REFLEX_OUT_OF_RANGE', `Reflex ratio must be >= 0 and < 1`);else {
	...
	set minFreq(value) {
		if (value < 1) throw new AudioMotionError('ERR_FREQUENCY_TOO_LOW', `Frequency values must be >= 1`);else {
			this._minFreq = value;
			this._precalculateBarPositions();
		}
	}
	...
	get isFullscreen() {
		if (document.fullscreenElement) return document.fullscreenElement === this._canvas;else if (document.webkitFullscreenElement) return document.webkitFullscreenElement === this._canvas;else return false;
	}
	get isOn() {
  ...
  */
	registerGradient(name, options) {
		if (typeof name !== 'string' || name.trim().length == 0) throw new AudioMotionError('ERR_GRADIENT_INVALID_NAME', 'Gradient name must be a non-empty string');

		if (typeof options !== 'object') throw new AudioMotionError('ERR_GRADIENT_NOT_AN_OBJECT', 'Gradient options must be an object');

		if (options.colorStops === undefined || options.colorStops.length < 2) throw new AudioMotionError('ERR_GRADIENT_MISSING_COLOR', 'Gradient must define at least two colors');

		this._gradients[name] = {};

		if (options.bgColor !== undefined) this._gradients[name].bgColor = options.bgColor;else this._gradients[name].bgColor = '#111';

		if (options.dir !== undefined) this._gradients[name].dir = options.dir;
  ...
  */
	setFreqRange(min, max) {
		if (min < 1 || max < 1) throw new AudioMotionError('ERR_FREQUENCY_TOO_LOW', `Frequency values must be >= 1`);else {
  ...
  */
	toggleFullscreen() {
		if (this.isFullscreen) {
			...
			if (this._canvas.requestFullscreen) this._canvas.requestFullscreen();else if (this._canvas.webkitRequestFullscreen) this._canvas.webkitRequestFullscreen();
		}
	}
  ...
  */
	_createLedMask() {
		...
		if (this._mode % 10 == 0 || !this._initDone) return;
		...
		for (let i = this._ledOptions.ledHeight; i < analyzerHeight; i += this._ledOptions.ledHeight + this._ledOptions.spaceV) this._ledsCtx.fillRect(0, i, this._canvas.width, this._ledOptions.spaceV);
	}
  ...
  */
	_draw(timestamp) {
		...
		if (!this.showBgColor) // use black background
			this._canvasCtx.fillStyle = '#000';else if (isLedDisplay) this._canvasCtx.fillStyle = '#111';else // use background color defined by gradient
		...
		if (!this.overlay || this.showBgColor) this._canvasCtx.fillRect(0, 0, this._canvas.width, isLumiBars ? this._canvas.height : analyzerHeight);
		...
		for (let i = 0; i < nBars; i++) {
			...
			if (bar.endIdx == 0) {
				...
				if (bar.factor) barHeight += (this._dataArray[bar.dataIdx + 1] - barHeight) * bar.factor;
			} else {
				// range of bins
				barHeight = 0;
				// use the highest value in the range
				for (let j = bar.dataIdx; j <= bar.endIdx; j++) barHeight = Math.max(barHeight, this._dataArray[j]);
			}

			// set opacity for lumi bars before barHeight value is normalized
			if (isLumiBars) this._canvasCtx.globalAlpha = barHeight / 255;

			if (isLedDisplay) // normalize barHeight to match one of the "led" elements
				barHeight = (barHeight / 255 * this._ledOptions.nLeds | 0) * (this._ledOptions.ledHeight + this._ledOptions.spaceV) - this._ledOptions.spaceV;else barHeight = barHeight / 255 * analyzerHeight | 0;
			...
			if (this._mode == 10) {
				...
				if (this._mode > 0) {
					if (isLedDisplay) posX += Math.max(this._ledOptions.spaceH / 2, this._barSpacePx / 2);else {
						if (this._barSpace == 0) {
							posX |= 0;
				...
				if (isLumiBars) {
					...
					this._canvasCtx.globalAlpha = 1;
				} else this._canvasCtx.fillRect(posX, analyzerHeight, adjWidth, -barHeight);
			}
			...
			if (bar.peak > 0) {
				if (this.showPeaks && !isLumiBars) {
					if (isLedDisplay) {
						this._canvasCtx.fillRect(posX, (this._ledOptions.nLeds - bar.peak / (analyzerHeight + this._ledOptions.spaceV) * this._ledOptions.nLeds | 0) * (this._ledOptions.ledHeight + this._ledOptions.spaceV), width, this._ledOptions.ledHeight);
					} else this._canvasCtx.fillRect(posX, analyzerHeight - bar.peak, adjWidth, 2);
				}

				if (bar.hold) bar.hold--;else {
					bar.accel++;
					bar.peak -= bar.accel;
				}
			}
		...
		if (this._mode == 10) {
			...
			// applies LEDs mask over the canvas
			if (this.overlay) this._canvasCtx.globalCompositeOperation = 'destination-out';
		...
		if (this.showScale) this._canvasCtx.drawImage(this._labels, 0, this._canvas.height - this._labels.height);
  ...
  */
	_findFrequencyBin(freq, func) {
		...
		if (!['floor', 'ceil'].includes(func)) func = 'round';
  ...
  */
	_generateGradients() {
		...
		Object.keys(this._gradients).forEach(key => {
			if (this._gradients[key].dir && this._gradients[key].dir == 'h') grad = this._canvasCtx.createLinearGradient(0, 0, this._canvas.width, 0);else grad = this._canvasCtx.createLinearGradient(0, 0, 0, analyzerHeight);

			if (this._gradients[key].colorStops) {
				this._gradients[key].colorStops.forEach((colorInfo, index) => {
					if (typeof colorInfo == 'object') grad.addColorStop(colorInfo.pos, colorInfo.color);else grad.addColorStop(index / (this._gradients[key].colorStops.length - 1), colorInfo);
				});
			}
	...
	_precalculateBarPositions() {
		...
		if (this._mode % 10 == 0) {
			...
			for (let i = minIndex; i <= maxIndex; i++) {
				...
				else if (this._analyzerBars.length) this._analyzerBars[this._analyzerBars.length - 1].endIdx = i;
			}
		} else {
			// Octave bands modes
			...
			if (this._mode == 8) groupNotes = 24;else if (this._mode == 7) groupNotes = 12;else if (this._mode == 6) groupNotes = 8;else if (this._mode == 5) groupNotes = 6;else groupNotes = this._mode; // for modes 1, 2, 3 and 4
			...
			while ((freq = c0 * root24 ** i) <= this._maxFreq) {
				if (freq >= this._minFreq && i % groupNotes == 0) temperedScale.push(freq);
				i++;
			}
			...
			temperedScale.forEach((freq, index) => {
				...
				if (prevBin > 0 && prevBin + 1 <= bin) idx = prevBin + 1;else idx = bin;
				...
				if (idx == prevIdx) {
					...
					if (nBars > 1) {
						for (let i = 1; i <= nBars; i++) this._analyzerBars[this._analyzerBars.length - i].factor = (nBars - i) / nBars;
					}
					prevIdx = idx;
					nBars = 1;
				}
				...
				if (temperedScale[index + 1] !== undefined) {
					...
					if (nextBin - bin > 1) prevBin += Math.round((nextBin - bin) / 2);
				}
		...
		for (const freq of freqLabels) {
			this._labelsCtx.fillText(freq >= 1000 ? `${freq / 1000}k` : freq, bandWidth * (Math.log10(freq) - minLog), this._labels.height * .75);
		}
	}
  ...
  */
	_setCanvas(reason) {
		...
		if (this._loRes) this._pixelRatio /= 2;
		...
		if (this._pixelRatio == 2 && window.screen.height <= 540) this._pixelRatio = 1;
  ...
  */
	_setProperties(options, defaults) {
		...
		if (defaults) options = Object.assign(defaults, options);

		for (const prop of Object.keys(options)) {
			if (callbacks.indexOf(prop) !== -1 && typeof options[prop] !== 'function') // check invalid callback
				this[prop] = undefined;else if (ignore.indexOf(prop) === -1) // skip ignored properties
...
var AudioMotion = function (Vue) {
    return class AudioMotion {
        ...
        init(el, audioCtx) {
            this.audioMotionObj = new AudioMotionAnalyzer(el, { audioCtx: audioCtx });
        }
        getAnalyzer() {
...
var AudioMotionComponent = (audioMotion => {
    return {
        props: {
            audioCtx: {
                type: AudioContext,
